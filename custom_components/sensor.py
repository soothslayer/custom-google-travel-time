...
class GoogleTravelTimeSensor(Entity):
    ...
    def update(self):
        ...
        if atime.split(".", 1)[0] == 'sensor':
            atime = self._get_location_from_entity(atime)
            _LOGGER.error("found a period in atime %s", atime)
            _LOGGER.error("atime is now %s", atime)
            self._options["arrival_time"] = atime
        else:
            _LOGGER.error("found no period in atime %s", atime)
        if atime is not None and ":" in atime:
            options_copy["arrival_time"] = convert_time_to_utc(atime)
            _LOGGER.error("converting atime:%s to utc and putting it in options_copy", atime)
        elif atime is not None:
            options_copy["arrival_time"] = atime
            _LOGGER.error("acopying atime: %s to options_copy", atime)
    ...
    def _get_location_from_entity(self, entity_id):
        _LOGGER.error("starting to get enitity from entity_id %s", entity_id)
        ...
        if entity_id.startswith("sensor."):
            _LOGGER.error("%s was a sensor. Getting state", entity_id)
            return entity.state
